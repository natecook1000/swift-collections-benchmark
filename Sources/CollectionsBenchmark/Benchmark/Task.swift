...
public struct Task<Input> {
  ...
  public let body: (Input) async -> Benchmark.TaskBody?
  ...
  public init(
    ...
    line: UInt = #line,
    body: @escaping (Input) async -> Benchmark.TaskBody?
  ) {
...
extension Task {
  public func measure(
    ...
    guard let instance = await self.body(input) else {
      _debug("Ignoring '\(title)' at size \(size)")
      return nil
    }

    let minDuration = options.minimumDuration
    let maxDuration = options.maximumDuration ?? Time(.infinity)
